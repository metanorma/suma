...
The `suma convert_jsdai` command converts JSDAI EXPRESS-G diagram outputs
...
[source,sh]
----
$ suma convert_jsdai XML_FILE IMAGE_FILE OUTPUT_DIR
----
...
[source,sh]
----
$ bundle exec suma convert_jsdai \
...
.To convert JSDAI outputs with JPEG image
[source,sh]
----
$ bundle exec suma convert_jsdai \
    diagrams/geometric_model_schemaexpg1.xml \
    diagrams/geometric_model_schemaexpg1.jpg \
    converted/
# => generates:
#    converted/geometric_model_schemaexpg1.svg
#    converted/geometric_model_schemaexpg1.exp
----
...
* `href` attribute → converted to EXPRESS cross-reference target in the `svgmap` block
* Sequential position → assigned as the numbered href in both SVG and `svgmap` list
