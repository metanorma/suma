...
Commands:
  ...
  suma validate-ascii EXPRESS_FILE_PATH  # Validate EXPRESS files for ASCII-only content
...
=== Validate-ASCII command

==== General

The `validate-ascii` command detects non-ASCII characters in EXPRESS files and reports on those exact lines, providing replacement suggestions.
